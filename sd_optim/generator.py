...
class Generator:
    ...
    async def generate(
        ...
        if webui_lower in ["a1111", "forge", "reforge"]: payload['batch_size'] = 1; payload['n_iter'] = 1
        elif webui_lower == "swarm":
             if 'images_per_prompt' in payload: payload['images_per_prompt'] = 1
             elif 'num_images' in payload: payload['num_images'] = 1
        ...
        try:
            ...
            async with session.post(api_url, json=payload) as resp: # Removed timeout here, handled by session
                ...
                for i, img_str in enumerate(r_json["images"]):
                    try:
                        if "," in img_str: img_data = base64.b64decode(img_str.split(",", 1)[1])
                        else: img_data = base64.b64decode(img_str)
        ...
        try:
            ...
            async with session.post(gen_api_url, json=payload) as response: # Removed timeout, handled by session
                ...
                for i, img_ref in enumerate(response_json["images"]):
                     ...
                     image_url_path = None
                     if isinstance(img_ref, dict) and 'url' in img_ref: image_url_path = img_ref['url']
                     elif isinstance(img_ref, str): image_url_path = img_ref
                     else: logger.warning(f"Unrecognized Swarm image ref: {img_ref}"); continue

                     if image_url_path.startswith('/'): view_url = f"{self.url.rstrip('/')}{image_url_path}"
                     else: view_url = f"{self.url.rstrip('/')}/View/{image_url_path}"
                     ...
                     try:
                         ...
                         async with session.get(view_url) as img_response: # Removed timeout, handled by session
                             if img_response.status != 200: logger.error(f"Failed Swarm image fetch {i+1} ({img_response.status})"); continue
                             image_bytes = await img_response.read()
                             image = Image.open(io.BytesIO(image_bytes))
                             yield image
                     except (aiohttp.ClientError) as fetch_err: logger.error(f"Error fetching Swarm image {i+1}: {fetch_err}")
                     except UnidentifiedImageError as img_err: logger.error(f"Error opening Swarm image {i+1}: {img_err}")

        # <<< REMOVED explicit TimeoutError catch >>>
        except aiohttp.ClientError as http_err: logger.error(f"HTTP error during Swarm generation: {http_err}", exc_info=False)
        except Exception as e: logger.error(f"Unexpected error in generate_swarm: {e}", exc_info=True)
    ...
    async def get_swarm_session(self, session: aiohttp.ClientSession) -> Optional[str]: # <<< ADD session parameter
        ...
        try:
            ...
            async with session.post(session_api_url, json={}) as response: # Removed timeout, handled by session
                if response.status != 200: logger.error(f"Swarm GetNewSession fail ({response.status})"); return None
                session_data = await response.json()
                session_id = session_data.get("session_id")
                if not session_id: logger.error(f"Swarm GetNewSession missing 'session_id'"); return None
                return session_id
        except Exception as e: logger.error(f"Error getting Swarm session ID: {e}"); return None
