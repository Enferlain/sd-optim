...
class LoadModelPayload(BaseModel):
    ...
    target_url: str | None = Field(None, title="Target URL (required for Swarm)")
...
def sd_optim_api_setup(_gui_unused, app: FastAPI):
        ...
        try:
            if webui_lower == "a1111":
                ...
                try:
                    ...
                    if not session_id:
                        raise ValueError("Swarm GetNewSession did not return session_id")
    ...
    async def unload_model_api(
        ...
        target_url: str | None = Query(None, title="Target URL (required for Swarm)"),
    ):
