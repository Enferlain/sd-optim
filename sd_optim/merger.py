...
class Merger:
    ...
    def _prepare_param_recipe_args(
        ...
        for base_param, block_dict in block_based_values_per_param.items():
             # --- NO LONGER ADDING DEFAULTS HERE ---
             if base_param in final_param_nodes: logger.warning(f"Param '{base_param}' conflict."); continue
             if not block_dict: logger.warning(f"No block values for '{base_param}'."); continue # Skip if empty
             if not self.custom_block_config: logger.error(f"Cannot make block node for '{base_param}': custom config missing."); continue
             if not target_model_node: logger.error(f"Cannot make convert node for '{base_param}': target missing."); continue
             try:
                 # Create literal node with ONLY the optimized block values
                 ...
                 logger.debug(f"Created CONVERT node for '{base_param}' ({len(block_dict)} optimized blocks).")
