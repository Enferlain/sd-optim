...
class BayesOptimizer(Optimizer):
    def __post_init__(self) -> None:
        ...
        self.setup_logging()
        self.optimizer: BayesianOptimization | None = None  # Initialize optimizer attribute
        self.optimization_start_time: float | None = None  # Track start time
            ...
            if load_log_file.is_file():
                try:
                    ...
                    if not self.cfg.optimizer.reset_log_file:
                        ...
                        logger.info(f"Loaded and transferred {len(self.previous_iterations)} iterations from {load_log_file}")
                    else:
    ...
    async def optimize(self) -> None:  # Changed to async
            ...
            try:
                ...
                for point in self.previous_iterations:
                    ...
                    if not self.cfg.optimizer.reset_log_file or not self.optimizer.space.params_registered(point["params"]):
                        self.optimizer.register(params=point["params"], target=point["target"])
                        loaded_count += 1
        ...
        if remaining_init_points > 0:
            ...
            continuous_bounds = {
                k: v for k, v in self.optimizer_pbounds.items() if not (isinstance(v, tuple) and v in [(0.0, 1.0), (1.0, 0.0)])
            }
            categorical_params = {
                k: v for k, v in self.optimizer_pbounds.items() if (isinstance(v, tuple) and v in [(0.0, 1.0), (1.0, 0.0)])
            }
