...
try:
    ...
    logger.warning(
        "Not running in A1111/Forge context. API endpoints will be inactive."
    )

    # Define dummy objects if absolutely needed for code analysis, but better to check the flag
    class DummyShared:
        device = "cpu"
        state = None  # Add state if needed
...
def sd_optim_api_setup(_gui_unused, app: FastAPI):
    ...
    if not A1111_FORGE_AVAILABLE:
        logger.warning(
            "A1111/Forge modules not loaded. Skipping API route registration."
        )
    ...
    async def load_model_api(payload: LoadModelPayload = Body(...)):
        ...
        logger.info(
            f"Received load request: UI='{payload.webui}', Path='{payload.model_path}'"
        )
        ...
        try:
            if webui_lower == "a1111":
                ...
                logger.debug("Loading for A1111 using sd_models...")
                ...
                logger.info(
                    f"A1111: Loaded model '{os.path.basename(payload.model_path)}'"
                )
                # --- End A1111 Logic ---

            elif webui_lower == "forge":
                # --- Forge Load Logic (from reference) ---
                logger.debug("Loading for Forge using forge_model_reload...")
                checkpoint_info = sd_models.CheckpointInfo(payload.model_path)
                # Set parameters for Forge loading
                # Ensure model_data exists and has forge_loading_parameters attribute
                if hasattr(sd_models, "model_data") and hasattr(
                    ...
                    if hasattr(main_entry, "checkpoint_change"):
                        main_entry.checkpoint_change(
                            os.path.basename(payload.model_path)
                        )
                    else:
                        logger.warning("main_entry.checkpoint_change not found.")
                    logger.info(
                        f"Forge: Loaded model '{os.path.basename(payload.model_path)}'"
                    )
                else:
                    logger.error(
                        "Forge loading structures (sd_models.model_data.forge_loading_parameters) not found."
                    )
                ...
                if not payload.target_url:
                    raise HTTPException(
                        status_code=400,
                        detail="target_url is required in request body for SwarmUI.",
                    )
                target_url = payload.target_url
                api_url = f"{target_url.rstrip('/')}/API/SelectModel"  # Ensure no double slash
                model_name = os.path.basename(
                    payload.model_path
                )  # Swarm likely uses name
                logger.debug(
                    f"Sending load request to Swarm: URL='{api_url}', Model='{model_name}'"
                )
                try:
                    # Get new session ID (synchronous requests for simplicity here)
                    session_resp = requests.post(
                        f"{target_url.rstrip('/')}/API/GetNewSession",
                        json={},
                        timeout=10,
                    )
                    ...
                    if not session_id:
                        raise ValueError(
                            "Swarm GetNewSession did not return session_id"
                        )

                    # Send SelectModel request
                    request_payload = {"session_id": session_id, "model": model_name}
                    response = requests.post(
                        api_url, json=request_payload, timeout=30
                    )  # Longer timeout for load
                    ...
                    logger.error(
                        f"Failed to communicate with SwarmUI at {target_url}: {req_err}"
                    )
                    raise HTTPException(
                        status_code=502, detail=f"Failed to contact SwarmUI: {req_err}"
                    )
                except Exception as swarm_e:
                    logger.error(f"Error during Swarm load request: {swarm_e}")
                    raise HTTPException(
                        status_code=500, detail=f"Swarm request failed: {swarm_e}"
                    )
                # --- End Swarm Logic ---

            elif webui_lower == "comfy":
                logger.warning(
                    "ComfyUI model loading via API requires workflow execution, not direct loading."
                )
                raise HTTPException(
                    status_code=501,
                    detail="ComfyUI loading not supported via this direct API call.",
                )
            else:
                raise HTTPException(
                    status_code=400,
                    detail=f"Invalid WebUI type specified: {payload.webui}",
                )
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Internal server error processing load request: {e}",
            )
    ...
    async def unload_model_api(
        ...
        target_url: Optional[str] = Query(
            None, title="Target URL (required for Swarm)"
        ),
    ):
        ...
        try:
            if webui_lower == "a1111":
                ...
                if hasattr(memory_management, "free_memory"):
                    ...
                    memory_management.free_memory(
                        1e30, shared.device, keep_loaded=[], free_all=True
                    )
                    logger.info(
                        "Forge: Called memory_management.free_memory(free_all=True)."
                    )
                else:
                ...
                if not target_url:
                    raise HTTPException(
                        status_code=400,
                        detail="target_url query parameter is required for SwarmUI.",
                    )
                ...
                try:
                    # Get new session ID
                    session_resp = requests.post(
                        f"{target_url}/API/GetNewSession", json={}, timeout=10
                    )
                    session_resp.raise_for_status()
                    session_id = session_resp.json().get("session_id")
                    if not session_id:
                    ...
                    response = requests.post(
                        api_url, json=request_payload, timeout=20
                ...
                logger.warning(
                    "ComfyUI model unloading not typically done via direct API call."
                )
                raise HTTPException(
                    status_code=501,
                    detail="ComfyUI unloading not supported via this API.",
                )
            else:
                raise HTTPException(
                    status_code=400, detail=f"Invalid WebUI type specified: {webui}"
                )
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Internal server error processing unload request: {e}",
            )
...
if script_callbacks and A1111_FORGE_AVAILABLE:
    ...
    logger.warning(
        "script_callbacks not available or not in A1111/Forge env. API endpoints will not be registered."
    )
