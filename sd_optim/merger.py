...
class Merger:
    ...
    def _create_model_nodes(self) -> List[ModelRecipeNode]:
        ...
        if not isinstance(model_paths_list, (list, ListConfig)):
            logger.warning(
                "'model_paths' in config is not a list. No model nodes will be created."
            )
            model_paths_list = []

        if not model_paths_list:
            logger.info(
                "No model paths provided, returning an empty list of model nodes."
            )
            return model_nodes

        if (
            not hasattr(self, "models_dir")
            or not self.models_dir
            or not self.models_dir.is_dir()
        ):
            logger.error(
        ...
        logger.info(
            f"Creating model nodes relative to base directory: {self.models_dir}"
        )
        for model_path_str in model_paths_list:
            try:
                if not isinstance(model_path_str, str):
                    logger.warning(
                        f"Skipping non-string path in model_paths: {model_path_str}"
                    )
                    continue
                ...
                if original_path.is_absolute():
                    if original_path.exists():
                        ...
                        logger.error(
                            f"Absolute model path not found: {original_path}. Skipping node."
                        )
                        continue
                else:
                    ...
                    if path_in_models_dir.exists():
                        ...
                        logger.error(
                            f"Relative model path not found: '{original_path}'. Skipping node."
                        )
                        continue
                ...
                logger.debug(
                    f"Resolved path '{resolved_path}' to recipe path '{path_for_recipe}'"
                )
                ...
                if isinstance(node, ModelRecipeNode):
                    ...
                    logger.warning(
                        f"Node created for path '{path_for_recipe}' was not a ModelRecipeNode. Skipping."
                    )
    ...
    def create_model_output_name(
        ...
        # --- Mode 3: RECIPE (This is the fixed logic) ---
            ...
            if recipe_node is not None:
                ...
                node_info = utils.get_info_from_target_node(
                    recipe_node, target_node_ref
                )
                ...
                if node_info:
                    ...
                    if len(model_names) >= 2:
                        ...
                        name_part = (
                            "optimized_merge"  # Should not happen if models exist
                        )
        ...
        if len(combined_name) > max_filename_len:
            logger.warning(
                f"Generated filename is too long. Truncating to {max_filename_len} characters."
            )
    ...
    def _select_base_model(self) -> Optional[recipe_nodes.ModelRecipeNode]:
        ...
        if not isinstance(base_model_index, int) or not (
            0 <= base_model_index < len(self.models)
        ):
            raise ValueError(
        ...
        try:
            ...
            with sd_mecha.open_input_dicts(base_model_node, [self.models_dir]):
                ...
                if base_model_node.model_config and (
                    "lora" in base_model_node.model_config.identifier
                    or "lycoris" in base_model_node.model_config.identifier
                ):
                    raise ValueError(
            ...
            raise ValueError(
                f"Could not verify base model '{base_model_node.path}'. Halting."
            ) from e
    ...
    def _get_conversion_context_node(self) -> recipe_nodes.ModelRecipeNode:
        ...
        if self.cfg.optimization_mode == "merge":
            ...
            )  # Our already-fixed _select_base_model
            if base_model:
                logger.info(
                    f"Using explicitly defined base_model '{base_model.path}' as conversion context."
                )
                return base_model
        ...
        if self.cfg.optimization_mode == "recipe":
            logger.debug(
                "Scanning recipe for the first valid base model to use as context..."
            )
            ...
            for model_node in visitor.models:
                try:
                    ...
                    logger.info(
                        f"Found suitable model '{model_node.path}' inside recipe to use as conversion context."
                    )
                    return model_node
                except ValueError:
                    logger.debug(
                        f"Skipping '{model_node.path}' as conversion context because it appears to be a LoRA."
                    )
                    continue
        ...
        raise RuntimeError(
            "Could not determine any suitable model to use for conversion context."
        )

    # We also need a small, reusable validator for the LoRA check.
    def _validate_node_is_not_lora(
        self, node: recipe_nodes.ModelRecipeNode, raise_error: bool = True
    ):
    ...
    def _slice_models(
        self, prepared_model_nodes: List[RecipeNodeOrValue], merge_method: MergeMethod
        ...
        for arg_type in input_types:
            ...
            if origin_type and (
                issubclass(origin_type, sd_mecha.extensions.merge_methods.StateDict)
                or issubclass(origin_type, torch.Tensor)
            ):
    ...
    def _handle_delta_output(
        ...
        input_spaces_kwargs = {
            k: node.merge_space for k, node in core_recipe_node.kwargs.items()
        }

        try:
            output_space = merge_method.get_return_merge_space(
                input_spaces_args, input_spaces_kwargs
            )
        except Exception as e:
            logger.error(
        ...
        if output_space == delta_space:
            if base_model_node:
                ...
                return sd_mecha.add_difference(
                    base_model_node, core_recipe_node, alpha=1.0
                )
            else:
                logger.warning(
    ...
    def _serialize_and_save_recipe(
        self, final_recipe_node: recipe_nodes.RecipeNode, model_path: Path
    ):
        ...
        try:
            ...
            ValueError
        ):  # Handle case where Hydra is not initialized (e.g., direct script run)
            log_dir = Path(os.getcwd()) / "logs" / "unknown_run"
            logger.warning(
                "Hydra config not found, saving recipe to default log directory."
            )
    ...
    def _prepare_model_recipe_args(
        ...
        conversion_target_node = (
            base_model_node
            if base_model_node
            else (initial_model_nodes[0] if initial_model_nodes else None)
        )
        ...
        for i, model_node in enumerate(initial_model_nodes):
            ...
            try:
                ...
                logger.error(
                    f"Could not infer config for model {original_path_for_logging}: {e}"
                )
                is_lora = False
            ...
            if is_lora:
                ...
                logger.info(
                    f"Converting LoRA '{original_path_for_logging}' to a delta..."
                )
                try:
            ...
            if is_delta_expected and current_node.merge_space != delta_space:
                ...
                if current_node is base_model_node:
                    ...
                    log_path = (
                        base_model_node.path
                        if isinstance(base_model_node, recipe_nodes.ModelRecipeNode)
                        else "the base model"
                    )
                    logger.warning(
                    ...
                    log_path = (
                        current_node.path
                        if isinstance(current_node, recipe_nodes.ModelRecipeNode)
                        else "a model node"
                    )
    ...
    def _prepare_param_recipe_args(
        ...
        for opt_param_name, info in param_info.items():
            if opt_param_name not in params:
                logger.warning(
                    f"Optimizer did not provide value for parameter '{opt_param_name}'. Skipping."
                )
                continue
            ...
            if target_type == "block":
                ...
                logger.warning(
                    f"Unknown target_type '{target_type}' for '{opt_param_name}'."
                )
                continue
            ...
            if strategy in ["all", "select"]:
                if not item_name:
                    logger.warning(
                        f"Missing 'item_name' for '{opt_param_name}' ({strategy})."
                    )
                    continue
                if target_type == "block":
                ...
                if not items_covered:
                    logger.warning(
                        f"Missing 'items_covered' for '{opt_param_name}' ({strategy})."
                    )
                    continue
        ...
        if not target_model_node:  # This check is now for a truly fatal error
            logger.error(
                "Cannot prepare parameter nodes: A conversion context model is missing."
            )
            return {}

        # Process each base parameter that has values from either blocks or keys
        all_base_params = set(block_based_values_per_param.keys()) | set(
            key_based_values_per_param.keys()
        )
        ...
        for base_param in all_base_params:
            ...
            if block_dict and not key_dict:
                if not self.custom_block_config:
                    logger.error(
                        f"Cannot make block node for '{base_param}': custom block config missing."
                    )
                    continue
                try:
                    literal_node = sd_mecha.literal(
                        block_dict, config=self.custom_block_config.identifier
                    )
                    converted_node = sd_mecha.convert(
                        literal_node, target_model_node, model_dirs=[self.models_dir]
                    )
                    final_param_nodes[base_param] = converted_node
                    logger.debug(
                        f"Created BLOCK-ONLY node for '{base_param}' ({len(block_dict)} blocks)."
                    )
                except Exception as e:
                    logger.error(
            ...
            # Case 2: Only key values
                ...
                if not self.base_model_config:
                    logger.error(
                        f"Cannot create key node for '{base_param}': base_model_config not loaded."
                    )
                    continue
                try:
                    literal_node = sd_mecha.literal(
                        key_dict, config=self.base_model_config.identifier
                    )
                    final_param_nodes[base_param] = literal_node
                    logger.debug(
                        f"Created KEY-ONLY node for '{base_param}' ({len(key_dict)} keys)."
                    )
                except Exception as e:
                    logger.error(
            ...
            # Case 3: Both block and key values - MERGE THEM with fallback
                ...
                try:
                    ...
                    logger.debug(
                        f"Created FALLBACK-MERGED node for '{base_param}' (blocks + keys, keys override)."
                    )
                except Exception as e:
                    logger.error(
            ...
            # Case 4: Neither (shouldn't happen, but just in case)
                ...
                logger.warning(
                    f"No values found for base parameter '{base_param}' - this shouldn't happen."
                )
        ...
        validated_custom_bounds = ParameterHandler.validate_custom_bounds(
            custom_bounds_config
        )
        ...
        for kwarg_name in unhandled_kwargs:
            if kwarg_name in validated_custom_bounds:
                ...
                logger.info(
                    f"Using fixed value for '{kwarg_name}' from custom_bounds: {fixed_value}"
                )
            else:
    ...
    def _execute_recipe(
        ...
        if fallback_index is None or fallback_index == -1:
            ...
            0 <= fallback_index < len(models_for_lookup)
        ):  # This check also now works for both modes.
            logger.error(
        ...
        try:
            if not self.models_dir or not self.models_dir.is_dir():
                raise FileNotFoundError(
                    "Merger.models_dir is not set or is not a valid directory."
                )
            ...
            sd_mecha.merge(
                ...
                merge_device=self.cfg.get(
                    "device", "cpu"
                ),  # Default merge device if not set
                merge_dtype=precision_mapping.get(
                    self.cfg.merge_dtype
                ),  # Get dtype object
                output_device="cpu",  # Keep saving to CPU
                output_dtype=precision_mapping.get(
                    self.cfg.save_dtype
                ),  # Get dtype object
    ...
    def _save_recipe_etc(
        ...
        try:
            ...
            if self.cfg.get("save_merge_artifacts", False):
                utils.save_merge_artifacts(
                    self.cfg, self, final_recipe_node, model_path, iteration
                )

        except Exception as e:
            logger.error(
                f"Error during post-merge saving operations: {e}", exc_info=True
            )
    ...
    def merge(
        ...
        logger.info(
            f"Starting merge process for iteration {iteration}"
        )  # <<< USE iteration
        ...
        if not model_path:  # Safety check
            ...
            model_path = (
                self.models_dir / f"merge_output_default_{cfg.merge_method}.safetensors"
            )
        ...
        merge_func = utils.resolve_merge_method(
            cfg.merge_method
        )  # Assumes utils exists

        # 3. Select base model (for delta subtraction, conversion context)
        base_model_node = self._select_base_model()

        logger.info(
            f"Selected base model node: {base_model_node.path if base_model_node else 'None'}"
        )
        ...
        core_recipe_node = merge_func(*sliced_model_nodes, **param_nodes).set_cache(
            cache
        )
    ...
    def recipe_optimization(
        ...
        """
        logger.info(
            f"--- Coordinating Recipe Optimization for Iteration {iteration} ---"
        )
        ...
        self.output_file = self.create_model_output_name(
            iteration=iteration, recipe_node=target_node
        )
        ...
        new_param_nodes = self._prepare_param_recipe_args(
            params, param_info, target_node.merge_method
        )

        # --- Step 3: SERIALIZATION (Call a simple utility) ---
        new_node_strings, param_to_final_idx = utils.serialize_nodes_for_rewrite(
            new_param_nodes
        )
        ...
        try:
            ...
            debug_path = (
                ...
                / f"iteration_{iteration}_failed_recipe.mecha"
            )
            debug_path.write_text(final_recipe_text, encoding="utf-8")
            raise ValueError(
                f"Final recipe deserialization failed. Saved debug recipe to {debug_path}: {e}"
            )

        # We now perform a DEEP injection of the cache into every merge node.
        logger.info(
            "Performing deep injection of the shared cache into the recipe graph..."
        )
    ...
    def layer_adjust(self, params: Dict, cfg: DictConfig) -> Path:  # Takes params
        ...
        if not output_path:
            ...
            model_name_for_fallback = (
                Path(cfg.model_paths[0]).stem if cfg.model_paths else "unknown_model"
            )
            output_path = (
                self.models_dir
                / f"layer_adjusted_{model_name_for_fallback}_fallback.safetensors"
            )
        ...
        if not model_path.is_file():
            resolved_path = (
                Path(cfg.models_dir) / model_path_str
            )  # Use original string path
            if resolved_path.is_file():
                model_path = resolved_path
                logger.info(f"Resolved layer_adjust model path to: {model_path}")
            else:
                raise FileNotFoundError(
                    f"Model for layer_adjust not found at '{model_path_str}' or '{resolved_path}'"
                )
        ...
        try:
            ...
            # Add support for other formats if needed (e.g., .ckpt)
                ...
                raise ValueError(
                    f"Unsupported file type for layer adjustment: {model_path.suffix}"
                )
        except Exception as e:
        ...
        logger.info(
            f"Determined model type for layer adjustment: {'SDXL' if is_xl_model else 'Non-SDXL'}"
        )
        ...
        try:
            ...
            modified_state_dict = utils.modify_state_dict(
                state_dict, params, is_xl_model
            )
        except Exception as e:
        ...
        try:
            ...
            logger.error(
                f"Failed to save adjusted model {output_path}: {e}", exc_info=True
            )
            raise
