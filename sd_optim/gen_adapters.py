...
class ComfyUIAdapter(BackendAdapter):
    ...
    def _inject_parameters(self, workflow: dict, payload: dict):
        ...
        for nid in latents:
            if "width" in payload:
                workflow[nid]["inputs"]["width"] = payload["width"]
            if "height" in payload:
                workflow[nid]["inputs"]["height"] = payload["height"]
        ...
        if samplers:
            ...
            if "cfg" in payload:
                inputs["cfg"] = payload["cfg"]
            if "sampler_name" in payload:
                inputs["sampler_name"] = payload["sampler_name"]
            if "scheduler" in payload:
                inputs["scheduler"] = payload["scheduler"]
